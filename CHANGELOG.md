...
- Groundwork for rewriting history from the Commits pane (no keys yet): reword, drop, edit, squash and fixup any commit of a linear range and fold pending `fixup!` commits, each as one `git rebase -i` with a todo ferrit generates and no editor opened. A merge commit in the range, a dirty worktree or an operation already in progress is refused before anything changes.
