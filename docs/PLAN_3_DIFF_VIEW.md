...
split text on "\ndiff --git "         -> one FileMeta per chunk
  within a chunk:
    ...
    split rest on "\n@@ "              -> one HunkMeta per chunk
      ...
      remaining lines                  -> body range (used verbatim in phase 6)
```
...
- Staging / unstaging (phase 6). The parse is shaped for it: patch = slice
  of `diff.text`.
...
- A stash entry's diff: Stash right side stays mock. Natural fit for phase 10
...
`--recount` shortcut keeps phase 6 to a few dozen lines.

Cursor/selection state across a background `refresh()`: phase 3's `right_key`
is file-level. Phase 6's line cursor should stick to the *same hunk* after an
