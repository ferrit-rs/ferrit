...
Implemented (not deferred): `draw_left_column` in `src/app/screens/mod.rs`.
...
- **M6** lazygit skin: the palette table above in `theme.rs`, `[N] Tab - Tab`
  ...
  `src/domain/git/model.rs` so G3..G5 swap the source without touching the UI.
...
real repo (`!app.is_mock()`), `draw_right_pane` (`src/app/screens/mod.rs`) falls straight
...
- **Every wordmark row needs the same width.** `toilet` right-pads every
  ...
  constant in `src/app/screens/mod.rs` stores its rows trimmed of that trailing padding
...
- **Tier selection**: `welcome_lines(width, height)` (`src/app/screens/mod.rs`) takes
...
- **Where it hooks**: `draw_right_pane` in `src/app/screens/mod.rs`, a dedicated
...
- **Tests**: `tests/diff_app.rs` — `files_pane_groups_nested_files_into_a_tree`
  ...
  trigger the tree) — `src/app/mod.rs`'s own unit tests and
