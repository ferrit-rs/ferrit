...
Scope is exactly `PLAN_0_GENERAL.md`'s phase 4 line: "stage / unstage at file,
...
Phase 3 was built for this on purpose (`PLAN_3_DIFF_VIEW.md` "Phase 4 staging
...
//! from `Diff` byte ranges to `git apply`. See docs/PLAN_4_STAGING.md.
...
`right_scroll: usize`. Phase 4 adds a cursor and a selection anchor, and a
...
across a refresh of an unchanged *file* selection; phase 4 keeps the *cursor*
...
- **granule hint** in the right-pane title: `Unstaged changes` becomes
  ...
  title the same way.
- **discard confirm**: a small centered modal, the phase 5 popup primitive
...
| conflicted file (`UU`) | `<space>` inert in phase 4; resolving conflicts is phase 9 |
...
Phase 5 commits the staged index: a message-input popup, `git commit`, plus
...
Deferred out of phase 4, revisit with their own phases or a follow-up:
...
- staging a conflicted file's resolution (phase 9, conflict flow).
- an undo of the last stage/discard via the reflog / `git stash` (see
  `INSPIRATION.md` git-time-machine); phase 10 territory.
